...
public class RoutesConfigurationDialog
		extends JDialog {
	...
	private void initComponents() {
		ArrayListModel<RouteGroup> routeGroups =
				routePersistence.getAllRouteGroups();
		routeGroupModel =
				new SelectionInList<RouteGroup>((ListModel) routeGroups);
		...
		routeNameField =
				BasicComponentFactory.createTextField(routeModel
		...
		TableColumn routedStateColumn =
				routeItemTable.getColumnModel().getColumn(1);
	...
	private final class RouteGroupSelectionHandler implements
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			RouteGroup selectedGroup =
					(RouteGroup) routeGroupList.getSelectedValue();
			if (selectedGroup == null)
				return;
			routesList.setSelectedIndex(-1);
			List<Route> routes =
					new ArrayList<Route>(selectedGroup.getRoutes());
	...
	private final class RouteSelectionHandler implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			List<RouteItem> routeItems =
					new ArrayList<RouteItem>(selectedRoute.getRouteItems());
	 ...
	 */
	private static final class RouteGroupListCellRenderer
	 ...
	 * selection is null, an empty text <code>""</code> is rendered.
	 */
	private static final class RouteListCellRenderer
			extends DefaultListCellRenderer {
	 ...
	 */
	private static final class RouteItemTableModel
			extends AbstractTableAdapter<RouteItem> {
	...
	private class AddRouteGroupAction
		...
		public void actionPerformed(ActionEvent e) {
			String newRouteGroupName =
					JOptionPane.showInputDialog(RoutesConfigurationDialog.this,
							"Enter the name of the new route group",
							"Add route group", JOptionPane.QUESTION_MESSAGE);
	...
	private class RemoveRouteGroupAction
			extends AbstractAction {
	...
	private class AddRouteAction
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				List<Route> routes =
						new ArrayList<Route>(selectedRouteGroup.getRoutes());
				routesModel.setList(routes);
			} catch (RoutePersistenceException e1) {
				ExceptionProcessor.getInstance().processException(e1);
			}
	...
	private class RemoveRouteAction
			extends AbstractAction {
	...
	private class AddRouteItemAction
			extends AbstractAction {
	...
	private class RecordRouteAction
			extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				try {
					...
					List<RouteItem> routeItems =
							new ArrayList<RouteItem>(route.getRouteItems());
					routeItemModel.setList(routeItems);
				} catch (RoutePersistenceException e1) {
					e1.printStackTrace();
				}
	...
	private class NumberEnteredAction
			extends AbstractAction {
	...
	private class RemoveRouteItemAction
			extends AbstractAction {
