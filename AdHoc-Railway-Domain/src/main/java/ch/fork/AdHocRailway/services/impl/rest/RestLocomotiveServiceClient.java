...
public interface RestLocomotiveServiceClient {
    ...
    @POST("/locomotive")
    LocomotiveGroup addLocomotive(@Body Locomotive locomotive);

    @PUT("/locomotive")
    LocomotiveGroup updateLocomotive(@Body Locomotive locomotive);

    @DELETE("locomotive/{id}")
    LocomotiveGroup deleteLocomotive(@Path("id") String id);
}
