...
int readconfig_LOOPBACK(xmlDocPtr doc, xmlNodePtr node, bus_t busnumber) {
	...
	strcpy(buses[busnumber].description, "GA GL FB SM POWER LOCK DESCRIPTION");
...
int init_gl_LOOPBACK(gl_state_t * gl) {
	...
	case 'M':
		...
		case 1:
			...
			break;
		case 2:
			return ((gl->n_fs == 14) || (gl->n_fs == 27) || (gl->n_fs == 28)) ?
	...
	case 'N':
		switch (gl->protocolversion) {
		case 1:
			return ((gl->n_fs == 14) || (gl->n_fs == 28) || (gl->n_fs == 128)) ?
					SRCP_OK : SRCP_WRONGVALUE;
			break;
		case 2:
			return ((gl->n_fs == 14) || (gl->n_fs == 28) || (gl->n_fs == 128)) ?
...
static void handle_sm_command(bus_t bus) {
	...
	case GET:
		...
		case PAGE:
		case CV:
			if ((smtmp.typeaddr >= 0) && (smtmp.typeaddr <= MAX_CV_NUMBER)) {
				smtmp.value = cv[smtmp.typeaddr];
			}
			break;
		case CV_BIT:
			if ((smtmp.typeaddr >= 0) && (smtmp.bit >= 0) && (smtmp.bit < 8)
					&& (smtmp.typeaddr <= MAX_CV_NUMBER)) {
				smtmp.value = (1 & (cv[smtmp.typeaddr] >> smtmp.bit));
			}
			break;
		}
		break;
	case SET:
		...
		setSM(bus, smtmp.type, smtmp.addr, smtmp.typeaddr, smtmp.bit,
				smtmp.value, 0);
		break;
	case VERIFY:
		switch (smtmp.type) {
		...
		case CV_BIT:
			if ((smtmp.typeaddr >= 0) && (smtmp.bit >= 0) && (smtmp.bit < 8)
					&& (smtmp.typeaddr <= MAX_CV_NUMBER)) {
				if (smtmp.value != (1 & (cv[smtmp.typeaddr] >> smtmp.bit)))
					smtmp.value = -1;
			} else {
				smtmp.value = -1;
			}
...
void *thr_sendrec_LOOPBACK(void *v) {
	...
	/*register cleanup routine */pthread_cleanup_push((void *) end_bus_thread, (void *) btd);
