...
//Logging ist grunds둻zlich ausgeschaltet
...
*/
	...
	//MFX UID SNIFFER
	//Init Timer2
	TCCR2B |= (1 << CS22);			//Prescaler = clk/64 => 3.2탎
...
ISR( INT2_vect)
{
	...
	case 27:
		...
		break;
	//SchienenAdresse SID
	//14 Bit lange Adresse vom Steuerger둻 - interessiert uns hier nicht!  ;-)
	case 28: //Bit 14
		if (bit0 == 1) {
			mfxSnifferState=30;
		}
	...
	case 119:
		...
		break;
	// jetzt k둴e noch CRC und ein paar Sync - das interessiert nicht!
	// ENDE
	case 120:
...
//	mmChangeDirection = 192;			//im alten M둹klin-Format wird Geschwindigkeit 1 als Richtungswechsel interpretiert. Als Trit codiert 1 -> 192
...
void initIdleLocoData() {
	...
	// Motorola High-Trit Korrektur (f웦 Adressen-Berechnung)
	// High = 3 (0x11)
	if (trit1 == 1)
		trit1 = 3;
	if (trit2 == 1)
...
void waitSolenoidAfterActivation() {
	if (timer0_interrupt > SOLENOID_WAIT_TIMERCYCLES) {  // timer0_interupt wird alle 13ms hochgez둯lt
...
void newMM2sendMM2() {
	...
	if (newLocoHiPrioQueue){
		...
		// Delta Spezial: nach Richtungs둵derung Geschwindigkeit wieder auf '0' setzen
...
void prepareRefreshLocoPacket(uint8_t protocol) {
	...
	if (protocol == MM2) {
		...
		uint16_t i = 0;
		//Pause auf Grund des vorg둵gigen Befehls bestimmen
