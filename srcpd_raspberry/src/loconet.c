/* $Id: loconet.c 1463 2010-03-01 20:21:37Z gscholz $ */
...
static int init_gl_LOCONET(bus_t busnumber, gl_state_t *, char *optData);
 ...
 *
 */
static int init_gl_LOCONET(bus_t busnumber, gl_state_t * gl, char *optData)
{
                    ...
                    switch (ln_packet[1]) {
                        case 0x0e:
                            ...
                            if (!isInitializedGL(btd->bus, addr)) {
                                cacheInitGL(btd->bus, addr, 'L', 1, 128, 9, NULL);
                            }
