...
#define ANZ_S88_WIEDERHOLUNGEN 3

typedef struct _DDL_S88_DATA {
    int number_fb[S88_MAXBUSSES];
    int max_number_fb;
    int port;
    int refresh;
    int clockscale;
    ...
    //Speicher für Mehrheitsentscheid Filterung
#ifdef RASPBERRYPI
    unsigned char s88dataBuffer[S88_MAXBUSSES][ANZ_S88_WIEDERHOLUNGEN][S88_MAXPORTSB];
    int spiFds[S88_MAXBUSSES]; //Die Handles für den SPI Filezugriff
#else
    unsigned char s88dataBuffer[S88_MAXBUSSES][S88_MAXPORTSB][8][ANZ_S88_WIEDERHOLUNGEN];
#endif
    int aktS88DataBufferIndex;
#ifdef __FreeBSD__
