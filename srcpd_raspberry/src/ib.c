 ...
 */
int init_gl_IB(bus_t busnumber, gl_state_t * gl, char *optData)
{
    gl->n_fs = 126;
    if (gl->n_func > 17) {
...
static void send_command_sm_IB(bus_t busnumber)
{
    ...
    if (!queue_SM_isempty(busnumber)) {
        ...
        __ib->last_typeaddr = smakt.cv_caIndex;
        __ib->last_bit = smakt.bit_index;
        ...
        switch (smakt.command) {
            case SET:
                if (smakt.addr == -1) {
                    switch (smakt.type) {
                        case REGISTER:
                            write_register_IB(busnumber, smakt.cv_caIndex,
                                              smakt.value);
                            break;
                        case CV:
                            write_cv_IB(busnumber, smakt.cv_caIndex,
                                        smakt.value);
                            break;
                        case CV_BIT:
                            write_cvbit_IB(busnumber, smakt.cv_caIndex,
                                           smakt.bit_index, smakt.value);
                            break;
                        case PAGE:
                            write_page_IB(busnumber, smakt.cv_caIndex,
                                          smakt.value);
                    }
                }
                else {
                    send_pom_IB(busnumber, smakt.addr, smakt.cv_caIndex,
                                smakt.value);
            ...
            case GET:
                switch (smakt.type) {
                    case REGISTER:
                        read_register_IB(busnumber, smakt.cv_caIndex);
                        break;
                    case CV:
                        read_cv_IB(busnumber, smakt.cv_caIndex);
                        break;
                    case CV_BIT:
                        read_cvbit_IB(busnumber, smakt.cv_caIndex,
                                      smakt.bit_index);
                        break;
                    case PAGE:
                        read_page_IB(busnumber, smakt.cv_caIndex);
                }
                break;
            case VERIFY:
                session_endwait(busnumber, 0);
...
static void check_status_gl_IB(bus_t busnumber)
{
    ...
    while (rr != 0x80) {
        ...
        if (!isInitializedGL(busnumber, gltmp.id)) {
            ...
            cacheInitGL(busnumber, gltmp.id, 'P', 1, 126, 5, NULL);
        }
