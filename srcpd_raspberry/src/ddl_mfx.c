 ...
 * @param msg Message, an die die Paramater gehängt werden sollen.
 */
void describeGLmfx(gl_state_t *gl, char *msg) {
  if (gl->protocol == 'X') {
...
/**
  ...
  @param prio true wenn Kommando priorisiert ist (Halt Koomandos).
  @return 0 == OK, 1 == Error
*/
int comp_mfx_loco(int address, int direction,
  ...
  if (nfuncs > 16) {
    gl_state_t glOld;
    if (cacheGetGL(busnumber, address, &glOld) == SRCP_OK) {
      int fxChange = func ^ glOld.funcs;
      int i;
      for (i=16; i<32; i++) {
        if ((fxChange & (1 << i)) != 0) {
          printf("COMP MFX Loco. Adr=%d, nfuncs=%d, Fx%d=%d\n", address, nfuncs, i, (func >> i) & 1);
          memset(packetstream, 0, PKTSIZE);
          pos = 0;
          addAdrBits(address, packetstream, &pos);
          addBits(0b100, 3, packetstream, &pos); //Funktion einzel
          addBits(i, 7, packetstream, &pos); //Funktionsnr
          addBits(0, 1, packetstream, &pos);
          addBits((func >> i) & 1, 1, packetstream, &pos); //On/Off
          addCRCBits(packetstream, &pos);
 ...
 * @param sid Schienenadresse, die dem Dekoder zugeordnet werden soll
 ...
 * Ein "init" einer MFX Lok führt immer zum Senden der MFX Adresszuordnung.
 * Wenn diese Lok bereits unter anderer Adresse vorhanden ist -> neue Adresse übernehmen, alte Lok löschen.
...
static void *thr_mfxManageThread(void *threadParam) {
  ...
  uint32_t firstUID = 0;
  //Nach jedem neuen Power-Up werden alle MFX Schienenadressen einmalig mit "assignSID" neu zugeordnet
  //(falls ein Dokoder sie vergessen hat...)
  bool sendSID_UID = true;
  for (;;) {
    if ((DDL_DATA*)buses[busnumber].power_state) {
