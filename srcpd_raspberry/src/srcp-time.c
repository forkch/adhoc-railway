...
void *thr_clock(void *v)
{
    ...
    while (true) {
      if ((((SERVER_DATA*)buses[0].driverdata) -> watchdog) &&
          ((time(NULL) - (((SERVER_DATA*)buses[0].driverdata) -> lastHeartBeatRx)) > 3)) {
        //Alle Busse mit GL -> Power OFF
        int i;
        for (i=0; i<MAX_BUSES ; i++) {
          if (bus_has_devicegroup(i, DG_GL) && bus_has_devicegroup(i, DG_POWER) &&
              (buses[i].power_state != 0)) {
            setPower(i, 0, "Watchdog Ausschaltung");
            syslog_bus(0, DBG_INFO, "Watchdog Ausschaltung");
          }
        }
      }
	...
	//Wenn aber ein Watchdog verlangt ist, ist der Timer immer notwendig!
        while ((distort.ratio_x == 0 || distort.ratio_y == 0) && (! (((SERVER_DATA*)buses[0].driverdata) -> watchdog))) {
            result =
        ...
        if ((distort.ratio_y == 0) || (distort.ratio_x == 0)) {
	  //Wir sind nur wegen Watchdog hier
	  sleeptime = 1000000;
	}
	else {
        ...
        if ((distort.ratio_y != 0) && (distort.ratio_x != 0)) {
	  //Wir nicht nur wegen Watchdog hier
