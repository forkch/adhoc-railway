...
static void handle_sm_command(bus_t bus)
{
    ...
    if (addr == 0 && smtmp.type == CV
        && (smtmp.cv_caIndex >= 0 && smtmp.cv_caIndex < 255)) {
        switch (smtmp.command) {
            case SET:
                syslog_bus(bus, DBG_INFO, "SM SET #%d %02X",
                           smtmp.cv_caIndex, smtmp.value);
                ...
                setSM(bus, smtmp.type, addr, smtmp.cv_caIndex,
                      smtmp.bit_index, smtmp.value, 0);
                break;
            case GET:
                syslog_bus(bus, DBG_INFO, "SM GET #%d", smtmp.cv_caIndex);
                snprintf(msg, sizeof(msg), "Q%02X\r", smtmp.cv_caIndex);
                writeString(bus, msg, 0);
