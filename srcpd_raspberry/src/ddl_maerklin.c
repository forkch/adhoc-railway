...
/*               January 2001                                  */
/*				 2019-01-02	m2 solenoid-addresses up to 968	   */
/*				 2020-04-03 m2 solenoid-address offset		   */
/*                                                             */
/*
...
int comp_maerklin_ms(bus_t busnumber, int address, int port, int action)
{
    ...
    address = address + 4;	/* m2 2020-04-03 */

    /* no special error handling, it's job of the clients */
    /* if (address < 1 || address > 324 || action < 0 || action > 1 || port < 0 || port > 1) {  m2 2019-01-02 */
    /*if (address < 1 || address > 968 || action < 0 || action > 1 || port < 0 || port > 1) {   m2 2019-01-02 */
    if (address < 1 || address > 972 || action < 0 || action > 1 || port < 0 || port > 1) {  /* m2 2020-04-03 */
        return 1;
    }
    p_packet = packet;
    id = ((address - 1) >> 2);
    ...
    /* compute address trits */
    /* for (i = 0; i < 4; i++) { m2 2019-01-02 */
     for (i = 0; i < 5; i++) { /* m2 2019-01-02 */
        j = id % 3;
        id = id / 3;
        switch (j) {
            case 0:
