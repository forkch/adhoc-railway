...
static bus_t register_bus(bus_t busnumber, xmlDocPtr doc, xmlNodePtr node)
{
    ...
    while (child != NULL) {
        ...
        else if (xmlStrcmp(child->name, BAD_CAST "mcp") == 0) {
#ifdef RASPBERRYPI
        	busnumber += readconfig_MCP_FB(doc, child, busnumber);
#else
            syslog_bus(0, DBG_ERROR, "no MCP on this platform available Bus :%d", busnumber);
#endif
        }
