/* $Id: srcp-server.c 1456 2010-02-28 20:01:39Z gscholz $ */
...
int readconfig_server(xmlDocPtr doc, xmlNodePtr node, bus_t busnumber)
{
    ...
    while (child != NULL) {
        ...
        else if (xmlStrcmp(child->name, BAD_CAST "watchdog") == 0) {
          txt = xmlNodeListGetString(doc, child->xmlChildrenNode, 1);
          if (txt != NULL) {
            if (xmlStrcmp(txt, BAD_CAST "yes") == 0) {
              syslog_bus(busnumber, DBG_INFO, "watchdog: %s", txt);
              __server->watchdog = true;
              __server->lastHeartBeatRx = time (NULL);
            }
            xmlFree(txt);
          }
        }
