...
#define SERVER_XBEE            13    /* Digi XBee Modules */
#define SERVER_MCP             14    /* MCP23S17 Port expander on GPIO*/
...
typedef struct _BUS
{
  ...
  int (*init_gl_func) (bus_t bus, gl_state_t *, char *);  /* called to check default init */
  ...
  void (*describe_gl_func) (gl_state_t *, char *);  /* called to check default init */
  ...
  char booster_state[8];	  /* Status eines einzelnen Boosters innerhalb eines Busses A=Active, S=Shortcut, O=Inactive */
} bus_data_t;
